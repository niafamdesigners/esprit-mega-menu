...
class EspritMegaMenu {
  ...
  setupElements() {
    ...
    this.megaMenuItems = document.querySelectorAll(
      ".esprit-menu__item--has-mega"
    );
    this.dropdownItems = document.querySelectorAll(
      ".esprit-menu__item--has-dropdown"
    );
  ...
  showMegaMenu(item, megaMenu) {
    ...
    const horizontalOffset =
      megaMenu.getAttribute("data-horizontal-offset") || "0";
    const isPositionAdjusted =
      megaMenu.getAttribute("data-position-adjusted") === "true";
    ...
    if (isPositionAdjusted) {
      ...
      console.log(
        "🎯 Smart positioning applied - no horizontal slide animation"
      );
    } else {
  ...
  showDropdown(item, dropdown) {
    ...
    // Get horizontal offset after positioning calculation
    const horizontalOffset =
      dropdown.getAttribute("data-horizontal-offset") || "0";
    const isPositionAdjusted =
      dropdown.getAttribute("data-position-adjusted") === "true";
    ...
    if (isPositionAdjusted) {
      ...
      console.log(
        "🎯 Smart positioning applied to dropdown - no horizontal slide animation"
      );
    } else {
      // Normal animation with transition
  ...
  setupMobileMenu() {
    ...
    this.menuItems.forEach((item) => {
      ...
      const hasDropdown = item.classList.contains(
        "esprit-menu__item--has-dropdown"
      );
  ...
  openMobileMenu() {
    ...
    const activeSubmenus = document.querySelectorAll(
      ".esprit-mega-menu--active, .esprit-dropdown--active"
    );
    if (activeSubmenus.length > 0) {
      console.warn(
        "⚠️ Found active submenus before opening main menu:",
    ...
    if (this.menuToggle) {
      const lines = this.menuToggle.querySelectorAll(
        ".esprit-nav__toggle-line"
      );
    ...
    const activeSubmenusAfter = document.querySelectorAll(
      ".esprit-mega-menu--active, .esprit-dropdown--active"
    );
    if (activeSubmenusAfter.length > 0) {
      console.error(
        "🚨 PROBLEM: Found active submenus after opening main menu:",
        activeSubmenusAfter.length
      );
      activeSubmenusAfter.forEach((submenu, index) => {
        console.log(
          `  - Unwanted active submenu ${index + 1}:`,
          submenu.className
        );
        console.log(
          `  - Parent item:`,
          submenu.closest(".esprit-menu__item")?.className
        );
      });
    }
  ...
  addMobileMenuHeader() {
    ...
    const existingHeader = this.menu.querySelector(
      ".esprit-mobile-menu-header"
    );
    if (existingHeader) {
  ...
  openMobileSubmenu(item) {
    ...
    if (hasMega) {
      ...
      console.log(
        "🔍 Looking for mega menu:",
        submenu ? "✅ Found" : "❌ Not found"
      );
    } else if (hasDropdown) {
    ...
    if (submenu) {
      ...
      const activeClass = hasMega
        ? "esprit-mega-menu--active"
        : "esprit-dropdown--active";
  ...
  focusNextMenuItem(currentIndex) {
    ...
    const nextLink =
      this.menuItems[nextIndex].querySelector(".esprit-menu__link");
    nextLink.focus();
  }
  ...
  focusPrevMenuItem(currentIndex) {
    ...
    const prevLink =
      this.menuItems[prevIndex].querySelector(".esprit-menu__link");
    prevLink.focus();
  }
